...
async function seed() {
  ...
  try {
    ...
    // Generate sectional-based performance data (every 200m)
    const generateSectionalData = () => {
      const sectionals = [];
      const totalDistance = 4800; // 4.8 km
      const sectionalDistance = 200; // 200m per sectional
      
      for (let i = 1; i <= totalDistance / sectionalDistance; i++) {
        const distance = i * sectionalDistance;
    ...
    const sectionals = generateSectionalData();
...
performanceData: {
        duration: 1602, // 26:42 in seconds
        distance: 4800, // 4.8 km in meters
        avgSpeed: sectionals.reduce((sum, s) => sum + s.avgSpeed, 0) / sectionals.length,
        maxSpeed: Math.max(...sectionals.map(s => s.maxSpeed)),
        ...
        speedHeartRate: {
          speedHeartRateChart: sectionals.flatMap((s, idx) => {
            // Generate 10 data points per sectional
            const points = [];
            for (let j = 0; j < 10; j++) {
      ...
      } as any, drizzle } from "drizzle-orm/mysql2";
import { eq } from "drizzle-orm";
import mysql from "mysql2/promise";
import * as schema from "../drizzle/schema";
import { userOrganizations } from "../drizzle/schema";
